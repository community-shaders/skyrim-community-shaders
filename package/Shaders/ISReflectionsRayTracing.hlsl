	...
	for (int i = 0; i < iterations; i++) {
		...
		if (saturate((raySample.z - iterationDepth) / SSRParams.y) > 0.0) {
			...
			if (fadeFactor > 0.0) {
				...
				float2 cameraMotionVector = MotionBlur::GetSSMotionVector2(positionWS);
				...
				if (!FrameBuffer::IsOutsideFrame(reprojectedRaySample.xy))
				{
					...
					alpha.w = saturate(alpha.w * SSRParams.z * SharedData::orderIndependentTransparencySettings.SSRAlphaScale);
				}

				// rgb channel in alpha texture is pre-multiplied
				float3 reflectionColor = color * (1 - alpha.w) + alpha.xyz * alpha.w ;
