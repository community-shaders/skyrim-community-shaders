...
StructuredBuffer<float> BufferAdaptation : register(t2);
...
#	elif defined(BLEND)
	...
	float3 ppColor = 0.0;
	{
		...
		float avgLuma = BufferAdaptation[0];
		inputColor *= 0.01 / clamp(avgLuma, AdaptationRange.x, AdaptationRange.y);		
		...
		linearColor = lerp(avgLuma, linearColor, Cinematic.z);
...
#		endif
	...
	psout.Color = float4(srgbColor, 1);
