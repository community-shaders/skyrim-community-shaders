...
void ThemeManager::SetupImGuiStyle(const Menu& menu)
{
	...
	if (isThemeCorrupted) {
		logger::warn("Theme appears corrupted, resetting to hardcoded defaults");
		// Reset to hardcoded default theme values
		const_cast<Menu*>(&menu)->GetSettings().Theme = Menu::ThemeSettings{};
	}
