...
//      func() runs → conductor executes Phase 2A
//      a. ExecutePassHook captures Phase 2A output to g_postPPCopy
//   2. After func() returns in BSImagespaceShaderHook:
//      a. Gets submit texture from bound RT
//      b. Evaluates DLSS on g_postPPCopy (post-PP intermediate)
//      c. Saves submit texture reference for deferred paste
//   3. Back in ExecutePassHook, Phase 5 (TAA + DRS) runs (passIdx=35):
//      a. TAA anti-aliases the entire submit texture (periphery benefits)
//      b. After Phase 5 returns: paste DLSS center onto submit texture
//         (overwrites TAA'd center with sharp DLSS output)
//   4. Engine continues: Orchestrator → Scaleform Display (UI) → Submit
//   5. Lock DRS + UpdateCameraData (in Main_PostProcessing::thunk after func())
//
...
namespace TAAReorder
{
	...
	// ─── BSImagespaceShader hook: DLSS eval after conductor, paste deferred ───
	...
	// ─── Saved submit texture for deferred paste (after Phase 5) ───
	inline winrt::com_ptr<ID3D11Texture2D> g_submitTexForPaste;
