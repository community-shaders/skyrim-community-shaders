...
void Streamline::CheckFrameConstants(sl::ViewportHandle p_viewport, uint32_t eyeIndex)
{
	...
	if (globals::game::isVR) {
		float nasalFrac = std::clamp(globals::features::upscaling.settings.vrDlssCropOffsetX, 0.0f, 0.3f);
		float vpScale = std::clamp(globals::features::upscaling.settings.vrDlssViewportScale, 0.5f, 1.0f);
	...
	if (globals::game::isVR && std::clamp(globals::features::upscaling.settings.vrDlssViewportScale, 0.5f, 1.0f) < 1.0f) {
		float invScale = 1.0f / std::clamp(globals::features::upscaling.settings.vrDlssViewportScale, 0.5f, 1.0f);
	...
	if (globals::game::isVR && TAAReorder::g_diagCounter == 0 && std::clamp(globals::features::upscaling.settings.vrDlssViewportScale, 0.5f, 1.0f) < 1.0f) {
...
void Streamline::Upscale(ID3D11Resource* a_upscalingTexture, ID3D11Resource* a_reactiveMask, ID3D11Resource* a_transparencyCompositionMask, ID3D11Resource* a_motionVectors, bool eye0Only)
{
	...
	if (globals::game::isVR) {
		...
		float vpScale = std::clamp(upscaling.settings.vrDlssViewportScale, 0.5f, 1.0f);
