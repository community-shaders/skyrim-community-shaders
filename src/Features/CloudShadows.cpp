...
void CloudShadows::SkyShaderHacks()
{
	if (overrideSky) {
		...
		// Per-layer: snapshot current face so cloud reads only shadows from above
		UINT sub = D3D11CalcSubresource(0, side, 1);
		context->CopySubresourceRegion(texCubemapCloudOccCopy->resource.get(), sub, 0, 0, 0,
			texCubemapCloudOcc->resource.get(), sub, nullptr);
		ID3D11ShaderResourceView* readSRV = texCubemapCloudOccCopy->srv.get();
		context->PSSetShaderResources(25, 1, &readSRV);
