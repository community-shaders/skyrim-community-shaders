...
{  // Release icon textures if loaded
	...
	// Clean up OAR user data
	if (auto* userData = static_cast<OAR_ImGuiUserData*>(ImGui::GetIO().UserData)) {
		delete userData;
...
void Menu::Load(json& o_json)
{
	...
	// Use hardcoded default theme - no theme preset switching
}
...
void Menu::Save(json& o_json)
{
	...
	// Remove Theme object from config (theme is hardcoded default only)
	o_json.erase("Theme");
...
void Menu::Init()
{
	...
	// Setup Platform/Renderer backends (using OAR's custom Win32 backend)
	ImGui_ImplWin32_Init(desc.OutputWindow);
	ImGui_ImplDX11_Init(globals::d3d::device, globals::d3d::context);

	// Calculate screen scale ratio for OAR backend mouse position mapping
	{
		RECT clientRect;
		::GetClientRect(desc.OutputWindow, &clientRect);
		float clientWidth = static_cast<float>(clientRect.right - clientRect.left);
		float clientHeight = static_cast<float>(clientRect.bottom - clientRect.top);
		float renderWidth = static_cast<float>(desc.BufferDesc.Width);
		float renderHeight = static_cast<float>(desc.BufferDesc.Height);

		auto* userData = new OAR_ImGuiUserData();
		if (clientWidth > 0.f && clientHeight > 0.f) {
			userData->screenScaleRatio.x = renderWidth / clientWidth;
			userData->screenScaleRatio.y = renderHeight / clientHeight;
		}
		imgui_io.UserData = userData;
		imgui_io.DisplaySize = ImVec2(renderWidth, renderHeight);
	}
