...
void LightingTemplateWidget::DrawBasicSettings()
{
	...
	if (ImGui::CollapsingHeader("Ambient & Directional", ImGuiTreeNodeFlags_DefaultOpen)) {
		ImGui::Spacing();
		if (MatchesSearch("Ambient Color") && DrawColorEdit("Ambient Color", settings.ambient))
			changed = true;
		if (MatchesSearch("Ambient Color"))
			ImGui::Spacing();
		if (MatchesSearch("Directional Color") && DrawColorEdit("Directional Color", settings.directional))
			changed = true;
	...
	if (ImGui::CollapsingHeader("Directional Settings", ImGuiTreeNodeFlags_DefaultOpen)) {
		ImGui::Spacing();
		if (MatchesSearch("Directional XY") && DrawSliderFloat("Directional XY", settings.directionalXY))
			changed = true;
		if (MatchesSearch("Directional XY"))
			ImGui::Spacing();
		if (MatchesSearch("Directional Z") && DrawSliderFloat("Directional Z", settings.directionalZ))
			changed = true;
		if (MatchesSearch("Directional Z"))
			ImGui::Spacing();
		if (MatchesSearch("Directional Fade") && DrawSliderFloat("Directional Fade", settings.directionalFade))
			changed = true;
	...
	if (ImGui::CollapsingHeader("Light Fade", ImGuiTreeNodeFlags_DefaultOpen)) {
		ImGui::Spacing();
		if (MatchesSearch("Light Fade Start") && DrawSliderFloat("Light Fade Start", settings.lightFadeStart))
			changed = true;
		if (MatchesSearch("Light Fade Start"))
			ImGui::Spacing();
		if (MatchesSearch("Light Fade End") && DrawSliderFloat("Light Fade End", settings.lightFadeEnd))
			changed = true;
	...
	if (ImGui::CollapsingHeader("Other", ImGuiTreeNodeFlags_DefaultOpen)) {
		ImGui::Spacing();
		if (MatchesSearch("Clip Distance") && DrawSliderFloat("Clip Distance", settings.clipDist))
			changed = true;
...
void LightingTemplateWidget::DrawFogSettings()
{
	...
	ImGui::Spacing();
	if (MatchesSearch("Fog Color Near") && DrawColorEdit("Fog Color Near", settings.fogColorNear))
		changed = true;
	if (MatchesSearch("Fog Color Near"))
		ImGui::Spacing();
	if (MatchesSearch("Fog Color Far") && DrawColorEdit("Fog Color Far", settings.fogColorFar))
		changed = true;
	if (MatchesSearch("Fog Color Far"))
		ImGui::Spacing();

	ImGui::Spacing();
	if (MatchesSearch("Fog Near") && DrawSliderFloat("Fog Near", settings.fogNear))
		changed = true;
	if (MatchesSearch("Fog Near"))
		ImGui::Spacing();
	if (MatchesSearch("Fog Far") && DrawSliderFloat("Fog Far", settings.fogFar))
		changed = true;
	if (MatchesSearch("Fog Far"))
		ImGui::Spacing();

	ImGui::Spacing();
	if (MatchesSearch("Fog Power") && DrawSliderFloat("Fog Power", settings.fogPower))
		changed = true;
	if (MatchesSearch("Fog Power"))
		ImGui::Spacing();
	if (MatchesSearch("Fog Clamp") && DrawSliderFloat("Fog Clamp", settings.fogClamp))
		changed = true;
