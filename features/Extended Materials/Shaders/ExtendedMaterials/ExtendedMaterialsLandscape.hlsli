...
#	define EMAT_LAND_TH_HEIGHT_NS_FOREACH(i, THDISP, COLTEX)                                                                       \
		if (activeMask & (1u << i)) {                                                                                               \
			...
			else heights[i] = ScaleDisplacement(COLTEX.SampleLevel(SampTerrainParallaxSampler, coords, mipLevels[i]).w, params[i]); \
		}
...
#	define EMAT_LAND_TH_HEIGHT_S_FOREACH(i, THDISP, COLTEX)                                                                                               \
		if (activeMask & (1u << i)) {                                                                                                                      \
			...
			else heights[i] = ScaleDisplacement(SampleHeightUnified(COLTEX, SampTerrainParallaxSampler, coords, mipLevels[i], sharedOffset).w, params[i]); \
		}
